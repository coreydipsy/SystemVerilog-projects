...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE bob [] = {reg+2,val};
	if(XSpi_Transfer(&SpiInstance, bob, NULL, 2) != 0){
		xil_printf("error111111111111111");
	}
	//write reg + 2 via SPI
	//write val via SPI
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE fuck385[nbytes+1];
	fuck385[0] = reg+2;
	for (int i=1; i<=nbytes; i++){
		fuck385[i] = data[i-1];
	}
	if (XSpi_Transfer(&SpiInstance, fuck385, NULL, nbytes+1) != 0){
		xil_printf("error2222222222222222");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data+nbytes);
	//psuedocode:
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE patrickstar[2];
	spongebob[0] = reg;
	if(XSpi_Transfer(&SpiInstance, spongebob, patrickstar, 2) != 0){
		xil_printf("error3333333333333333333333");
	}
	//select MAX3421E (
	//write reg via SPI
	//read val via SPI
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE IShouldDropOut[nbytes+1];
	BYTE ECEtoopainful[nbytes+1];
	IShouldDropOut[0] = reg;
	if(XSpi_Transfer(&SpiInstance, IShouldDropOut, ECEtoopainful, nbytes+1) !=0){
		xil_printf("error444444444444444444444444444");
	}
	for (int i=1; i<=nbytes; i++){
		data[i-1] = ECEtoopainful[i];
	}
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	//write reg via SPI
